...
def choose_audio_output(outputs: list[str]) -> Optional[str]:
    return resolve_port(DEFAULT_OUTPUT_PORT, outputs)
...
class LiveStudioController:
    ...
    def refresh_devices(self, keep_manual: bool = False) -> DeviceState:
        ...
        selected_output = choose_audio_output(outputs)
    ...
    async def start_session(self, requested_input: Optional[str] = None) -> None:
        ...
        resolved_output = choose_audio_output(self.devices.outputs)
        if resolved_output is None:
            self.session.last_error = "Python_OUT not found. Start loopMIDI and create Python_OUT."
            await self.broadcast_session()
            await self.manager.broadcast({"type": "error", "message": self.session.last_error})
            return
